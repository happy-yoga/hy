...
function heart (rSize, rLeft, rTime, rNum) {
  return (
...
setInterval(function () {
  ...
  var rTime = Math.floor(Math.random() * 15) + 5

  var bgHeart = document.querySelector('.bg-heart')
  bgHeart.innerHTML =
    'happy body. happy life.' +
    heart(rSize, rLeft, rTime, 0) +
    heart(rSize - 10, rLeft, rTime + 5, rNum) +
    heart(
      rSize - 20,
      rLeft + Math.floor(Math.random() * 40) + 1,
      rTime + 7,
      rNum
    ) +
    heart(
